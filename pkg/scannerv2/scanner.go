...
func (s *Scanner) processRequests(
	...
	for {
		...
		case r := <-chan_requests_in:
			...
			_, err := s.TrackerRequests.Update(r.ID, KeyCodePending, "")
			if err != nil {
				chan_errors_out <- err
				continue
			}
...
// processResponses() method processes responses for requests generated by
// the scan.
func (s *Scanner) processResponses(
	...
	for {
		...
		case r := <-chan_responses_in:
			...
			// update TrackerRequests for the associated request (ID) as complete
			_, err := s.TrackerRequests.Update(r.ID, KeyCodeComplete, "")
			if err != nil {
				chan_errors_out <- err
				continue
			}
