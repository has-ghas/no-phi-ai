...
func NewScanRepository(in NewScanRepositoryInput) (*ScanRepository, error) {
	...
	// create a new tracker.KeyTracker for tracking scanned commits
	tracker_commits, t_err := tracker.NewKeyTracker(tracker.ScanObjectTypeCommit, logger)
	if t_err != nil {
		return nil, errors.Wrap(t_err, ErrMsgScanRepositoryCreate)
	}
	// create a new KeyTracker for tracking scanned files
	tracker_files, t_err := tracker.NewKeyTracker(tracker.ScanObjectTypeFile, logger)
	if t_err != nil {
		return nil, errors.Wrap(t_err, ErrMsgScanRepositoryCreate)
	}
